"""
Cassandra BioHarvest Agent - State Management
Defines all state data structures and operations for clinical literature search workflow.
"""
...
@dataclass
class Search:
    """Individual search result state for PubMed/Clinical Trials queries."""
    query: str = ""                    # Search query string
    url: str = ""                      # Result URL (PubMed link, NCT registry, etc.)
    title: str = ""                    # Result title (paper title or trial name)
    content: str = ""                  # Retrieved content (abstract or trial summary)
...
@dataclass
class Research:
    """Research process state for biomedical literature analysis."""
    search_history: List[Search] = field(default_factory=list)     # Search history log (all queries executed)
    latest_summary: str = ""                                       # Current section summary (latest iteration)
    reflection_iteration: int = 0                                  # Reflection iteration count (self-critique cycles)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Research":
        """Create Research object from dictionary data for deserialization"""
...
@dataclass
class Paragraph:
    """State of a single section in the biomedical due diligence report."""
    title: str = ""                                                # Section title (e.g., "Clinical Trial Failures")
    content: str = ""                                              # Expected content outline (initial planning)
    research: Research = field(default_factory=Research)          # Research progress tracker
    order: int = 0                                                 # Section order in report
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Paragraph":
        """Create Paragraph object from dictionary data for deserialization"""
...
@dataclass
class State:
    """Complete biomedical due diligence report state."""
    query: str = ""                                                # Original user query (e.g., "Analyze nivolumab safety")
    report_title: str = ""                                         # Report title
    paragraphs: List[Paragraph] = field(default_factory=list)     # Report sections (ordered list)
    final_report: str = ""                                         # Final compiled Markdown report
    is_completed: bool = False                                     # Report completion flag
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "State":
        """Create State object from dictionary data for deserialization"""
