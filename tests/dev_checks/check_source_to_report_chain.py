...
from src.engines.harvest.core.field_mappings import project_source_payloads_for_frontend
...
class SpyReportAgent:
    ...
    def _write(payload: Dict[str, Any]) -> StubReportOutput:
        ...
        evidence_rows = payload.get("evidence_data") or []
        if not evidence_rows:
            raise AssertionError("writer payload has no evidence_data rows")

        missing_anchor_rows = []
        for idx, row in enumerate(evidence_rows):
            if not isinstance(row, dict):
                missing_anchor_rows.append(idx)
                continue
            if not str(row.get("doc_id") or "").strip() or not str(row.get("paragraph_id") or "").strip():
                missing_anchor_rows.append(idx)

        if missing_anchor_rows:
            raise AssertionError(
                "Doc_ID/paragraph_id anchors missing in evidence_data rows: "
                f"{missing_anchor_rows[:8]}"
            )
...
def run_stubbed_chain_check(query: str) -> bool:
    ...
    try:
        ...
        final_state = supervisor.run_cassandra_workflow(user_query=query, pdf_paths=[])
    finally:
