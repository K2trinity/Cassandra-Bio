...
class ChartInjector:
    """从 evidence 数据自动创建图表/表格 block 并注入 IR 章节"""
    ...
    def enrich(
        ...
        Args:
            doc: IR 文档
            evidence_data: 从 EvidenceEngine 输出的结构化证据
            forensic_data: 从 ForensicEngine 输出的取证结果
        ...
        for chapter in doc.chapters:
            ...
            if not has_visual:
                self._auto_inject_visuals(chapter, evidence_data, forensic_data)

        # 注入文献图表
        if extracted_figures:
            self._inject_literature_figures(doc, extracted_figures)

        # 生成全局统计仪表盘（如果存在 evidence 汇总数据）
        if evidence_data:
            self._inject_summary_dashboard(doc, evidence_data)
            self._inject_core_biomedical_charts(doc, evidence_data)
    ...
    def _auto_inject_visuals(
        ...
        elif any(kw in title_lower for kw in ["forensic", "integrity", "image", "manipulation"]):
            chart = self._make_forensic_chart(forensic)
            if chart:
                chapter.blocks.append(chart)
    ...
    def _make_forensic_chart(self, forensic: Dict) -> Optional[ChapterBlock]:
        """图像取证结果饼图"""
        results = forensic.get("results", forensic.get("forensic_evidence", []))
        if not results:
            return None

        categories = {"Clean": 0, "Suspicious": 0, "Flagged": 0}
        for r in results:
            verdict = str(r.get("verdict", r.get("classification", "clean"))).lower()
            if "suspicious" in verdict or "tamper" in verdict:
                categories["Suspicious"] += 1
            elif "flag" in verdict or "high" in verdict:
        ...
        return ChapterBlock(
            ...
            content={
                "type": "pie",
                "title": "Image Forensic Analysis Results",
                "caption": f"Classification of {sum(data)} analyzed images",
                "labels": labels,
                "datasets": [{
                    "label": "Images",
                    "data": data,
                    "backgroundColor": ["#50C878", "#FFB347", "#E85D75"][:len(labels)],
                }],
            },
            metadata={"auto_generated": True},
        )
