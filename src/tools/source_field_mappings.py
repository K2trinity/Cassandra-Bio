"""
Source field whitelist and mapping tables.

This module centralizes:
1) Whitelists for ClinicalTrials / NCBI / openFDA payload projection.
2) Canonical mapping table for schema hardening and frontend rendering.
3) Utility functions to build a compact, UI-friendly payload.
"""
...
def _get_nested_value(record: Dict[str, Any], path: str) -> Any:
    """Safely fetch nested values from dict using dot-path."""
    parts = path.split(".")
    current: Any = record

    for idx, key in enumerate(parts):
        if isinstance(current, list):
            # When path traverses arrays, project remaining path for each dict item.
...
def _pick_fields(record: Dict[str, Any], whitelist: List[str]) -> Dict[str, Any]:
    """Pick fields from record, supporting dot-path selectors."""
...
def project_source_payloads_for_frontend(source_payloads: Dict[str, Any], max_items: int = 50) -> Dict[str, Any]:
    """
    Build compact frontend payload based on explicit field whitelists.

    Args:
        source_payloads: Raw source payloads from MultiSourceHarvester
        max_items: Max list length per source segment

    Returns:
        A compact dict that is stable for frontend table rendering.
    """
...
def get_mapping_table(source: Optional[str] = None) -> Dict[str, Any]:
    """Return mapping table for all sources or a single source."""
    if source:
