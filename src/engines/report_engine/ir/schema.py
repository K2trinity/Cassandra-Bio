...
class ChapterBlock:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ChapterBlock":
        if not isinstance(data, dict):
            return cls(type=BlockType.PARAGRAPH, content=str(data))

        raw_type = str(data.get("type") or "paragraph").lower()
        aliases = {
            "chart": BlockType.CHART,
            "graph": BlockType.CHART,
            "visualization": BlockType.CHART,
            "wordcloud": BlockType.WORDCLOUD,
            "word_cloud": BlockType.WORDCLOUD,
        ...
        block_type = aliases.get(raw_type, BlockType(raw_type) if raw_type in BlockType._value2member_map_ else BlockType.PARAGRAPH)
        return cls(
            type=block_type,
            content=data.get("content"),
            level=data.get("level"),
            metadata=data.get("metadata") if isinstance(data.get("metadata"), dict) else {},
            block_id=data.get("block_id") or data.get("blockId"),
        )
...
class Chapter:
    ...
    def word_count(self) -> int:
        ...
        for block in self.blocks:
            total += _estimate_words(block.content)
        return total
...
def _estimate_words(value: Any) -> int:
    if value is None:
        return 0
    ...
    if isinstance(value, dict):
        ...
        for item in value.values():
            total += _estimate_words(item)
        return total
    if isinstance(value, list):
        return sum(_estimate_words(item) for item in value)
    return 1
