...
from ..ir.schema import ENGINE_AGENT_TITLES
...
class HTMLRenderer:
    ...
    def __init__(self, config: Dict[str, Any] | None = None):
        ...
        if not llm_repair_fns:
            logger.debug("HTMLRenderer: 图表LLM修复未启用（使用本地修复）")
        else:
  ...
  <script{defer_attr}>
    ...
    def render(
        ...
        """
        if hasattr(document_ir, "chapters") and hasattr(document_ir, "to_dict"):
          return self._render_legacy_document(document_ir)
        if hasattr(document_ir, "to_dict"):
          document_ir = document_ir.to_dict()
        self.document = document_ir or {}

        # 使用统一的 ChartReviewService 进行图表校验与修复
    ...
    def _render_legacy_document(self, document_ir: Any) -> str:
        chapters = getattr(document_ir, "chapters", []) or []
        title = getattr(document_ir, "title", None) or "报告"
        subtitle = getattr(document_ir, "subtitle", None) or ""
        toc_items = []
        body_parts = []
        ...
        for chapter in chapters:
            chapter_title = getattr(chapter, "title", "章节")
            slug = getattr(chapter, "slug", chapter_title.lower().replace(" ", "-"))
            toc_items.append(f'<li><a href="#{html.escape(slug)}">{html.escape(chapter_title)}</a></li>')
            chapter_html = [f'<section id="{html.escape(slug)}" class="chapter">']
            chapter_html.append(f'<h2>{html.escape(chapter_title)}</h2>')
            for block in getattr(chapter, "blocks", []) or []:
                block_type = getattr(block.type, "value", str(block.type)).lower() if hasattr(block, "type") else ""
                if block_type == "heading":
                    level = getattr(block, "level", 2) or 2
                    text = html.escape(str(getattr(block, "content", "")))
                    chapter_html.append(f'<h{level}>{text}</h{level}>')
                elif block_type == "paragraph":
                    text = html.escape(str(getattr(block, "content", "")))
                    chapter_html.append(f'<p>{text}</p>')
                elif block_type == "chart":
                    chapter_html.append(_render_chart(block, chart_index=chart_index))
                    chart_index += 1
                elif block_type == "wordcloud":
                    chapter_html.append(_render_wordcloud(block, wc_index=wordcloud_index))
                    wordcloud_index += 1
                else:
                    chapter_html.append(f'<div class="unknown-block">{html.escape(str(getattr(block, "content", "")))}</div>')
            chapter_html.append("</section>")
            body_parts.append("\n".join(chapter_html))

        toc_html = "<nav id=\"toc-container\"><ul>" + "".join(toc_items) + "</ul></nav>"
        init_script = """
<script>
function initCharts() { return window.__cassandraCharts || []; }
function initWordclouds() { return window.__cassandraWordclouds || []; }
document.addEventListener('DOMContentLoaded', function() {
  initCharts();
  initWordclouds();
});
...
"""
        return (
            "<!DOCTYPE html>\n<html lang=\"zh-CN\">\n<head>\n"
            f"<meta charset=\"UTF-8\"><title>{html.escape(str(title))}</title>\n"
            '<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>\n'
            "</head>\n<body>\n"
            f"<h1>{html.escape(str(title))}</h1>\n"
            f"<p class=\"subtitle\">{html.escape(str(subtitle))}</p>\n"
            f"{toc_html}\n"
            + "\n".join(body_parts)
            + f"\n{init_script}\n</body>\n</html>"
        )
...
""".strip()
    ...
    def _strip_order_prefix(text: str) -> str:
        ...
        prefix_patterns = (
            r"^第\s*[一二三四五六七八九十百千万0-9]+\s*[章节部分篇]\s*[:：.、-]?\s*",
            r"^[一二三四五六七八九十百千万]+[、.．]\s*",
            r"^\d+(?:\.\d+)*[、.．)]\s*",
        )
        for pattern in prefix_patterns:
            cleaned = re.sub(pattern, "", stripped, count=1)
            if cleaned != stripped:
    ...
    def _render_paragraph(self, block: Dict[str, Any]) -> str:
        ...
        align_attr = (
            f' style="text-align: {self._escape_attr(align)};"'
            if align in {"left", "center", "right", "justify"}
            else ""
        )
        return f"<p{align_attr}>{inlines}</p>"
    ...
    def _render_table(self, block: Dict[str, Any]) -> str:
        ...
        header_cells = self._flatten_nested_cells(rows[0].get("cells", [])) if rows else []
        has_header_row = bool(header_cells) and all(
            cell.get("header") or cell.get("isHeader") for cell in header_cells
        )
        column_labels = [
            self._table_cell_text(cell) or f"Column {index + 1}"
            for index, cell in enumerate(header_cells)
        ] if has_header_row else []
        column_keys = [
            self._safe_css_class_token(column.get("key"))
        ...
        def render_row(row: Dict[str, Any]) -> str:
            ...
            for index, cell in enumerate(cells):
                ...
                if index < len(column_labels):
                    attr.append(f'data-column="{self._escape_attr(column_labels[index])}"')
                if index < len(column_keys) and column_keys[index]:
                    attr.append(f'data-column-key="{self._escape_attr(column_keys[index])}"')
        ...
        header_html = f"<thead>{render_row(rows[0])}</thead>" if has_header_row else ""
        body_rows = rows[1:] if has_header_row else rows
        rows_html = "".join(render_row(row) for row in body_rows)
        caption = block.get("caption")
        caption_html = f"<caption>{self._escape_html(caption)}</caption>" if caption else ""
        table_classes = self._table_class_tokens(table_class)
        layout_token = self._safe_css_class_token(layout_hint)
        if layout_token:
            table_classes.append(f"layout-{layout_token}")
        table_attr = f' class="{self._escape_attr(" ".join(table_classes))}"' if table_classes else ""
        layout_attr = f' data-layout="{self._escape_attr(layout_hint)}"' if layout_hint else ""
        colgroup_html = self._render_table_colgroup(colgroup)
        wrapper_classes = ["table-wrap"]
        if has_layout_hint:
            wrapper_classes.append("table-wrap--wide")
        if layout_token:
            wrapper_classes.append(f"table-wrap--layout-{layout_token}")
        return (
            f'<div class="{self._escape_attr(" ".join(wrapper_classes))}"{layout_attr}>'
            f"<table{table_attr}{layout_attr}>"
            f"{caption_html}{colgroup_html}{header_html}<tbody>{rows_html}</tbody></table></div>"
        )

    def _table_class_tokens(self, class_value: Any) -> List[str]:
        return [
            token
            for token in (
                self._safe_css_class_token(part)
                for part in self._safe_text(class_value).split()
            )
            if token
        ]

    def _safe_css_class_token(self, value: Any) -> str:
        token = self._safe_text(value).strip().lower()
        token = re.sub(r"[^a-z0-9_-]+", "-", token)
        return token.strip("-")

    def _table_cell_text(self, cell: Dict[str, Any]) -> str:
        pieces: List[str] = []
        for block in cell.get("blocks") or []:
            ...
            if block.get("type") == "paragraph":
                for inline in block.get("inlines") or []:
                    if isinstance(inline, dict):
                        pieces.append(self._safe_text(inline.get("text")))
            elif block.get("text") is not None:
                pieces.append(self._safe_text(block.get("text")))
    ...
    def _apply_cached_review_stats(self, block: Dict[str, Any]) -> None:
        """
        在已校验过的图表上重新累计统计信息，避免重复修复。

        当渲染流程重置了统计但图表已经校验过（_chart_reviewed=True），
    ...
    def review_and_patch_document(
        ...
        *,
        ...
        """
        全局校验并修复图表，将修复结果回写到原始 IR，避免多次渲染重复修复。
...
p {{ /* 含义：段落样式；设置：在本块内调整相关属性 */
  ...
  text-align: left; /* 含义：默认使用左对齐，减少大段正文的块状感；设置：按需调整数值/颜色/变量 */
}} /* 结束 p */
...
caption {{ /* 含义：表格标题；设置：保持标题与表格关联 */
  caption-side: top; /* 含义：表格标题位置；设置：标题显示在表格上方 */
  text-align: left; /* 含义：文本对齐；设置：左对齐便于扫读 */
  font-weight: 650; /* 含义：字重；设置：增强表格标题层级 */
  margin-bottom: 8px; /* 含义：标题与表格的距离 */
  color: var(--text-color); /* 含义：文字颜色；设置：跟随主题 */
}} /* 结束 caption */
table th, table td {{ /* 含义：表格单元格；设置：在本块内调整相关属性 */
  padding: 12px; /* 含义：内边距，控制内容与容器边缘的距离；设置：按需调整数值/颜色/变量 */
  border: 1px solid var(--border-color); /* 含义：边框样式；设置：按需调整数值/颜色/变量 */
  vertical-align: top; /* 含义：长文本单元格顶部对齐 */
  overflow-wrap: anywhere; /* 含义：允许长分子代号、NCT号或英文长串在任意点断行 */
  word-break: normal; /* 含义：保留自然断词，避免逐字硬拆 */
  hyphens: auto; /* 含义：英文长词在支持环境中自动连字符换行 */
}} /* 结束 table th, table td */
table th {{ /* 含义：table th 样式区域；设置：在本块内调整相关属性 */
  background: rgba(0,0,0,0.03); /* 含义：背景色或渐变效果；设置：按需调整数值/颜色/变量 */
}} /* 结束 table th */
.table-wrap table p {{ /* 含义：表格内段落；设置：压缩单元格内部段落节奏 */
  margin: 0; /* 含义：移除全局段落外边距，避免PDF表格单元格变成大块空白 */
  line-height: 1.35; /* 含义：长文本行高；设置：在可读性和密度之间平衡 */
}} /* 结束 .table-wrap table p */
.table-wrap table p + p {{ /* 含义：表格内连续段落；设置：保留轻量层次 */
  margin-top: 0.35em; /* 含义：连续段落间距 */
}} /* 结束 .table-wrap table p + p */
.table-wrap--wide th,
.table-wrap--wide td {{ /* 含义：宽表单元格；设置：降低密度压力 */
  padding: 8px 9px; /* 含义：宽表内边距；设置：比普通表格更紧凑 */
  font-size: 0.86rem; /* 含义：宽表字号；设置：减少拥挤与溢出 */
  line-height: 1.32; /* 含义：宽表行高；设置：适配长文本换行 */
}} /* 结束 .table-wrap--wide td */
.table-wrap--wide thead th {{ /* 含义：宽表表头；设置：固定层级并减少高度 */
  font-size: 0.8rem; /* 含义：表头字号 */
  line-height: 1.2; /* 含义：表头行高 */
}} /* 结束 .table-wrap--wide thead th */
...
.table-wrap--wide {{ /* 含义：PDF宽表容器；设置：允许宽表自然分页而不是整体挤压 */
  break-inside: auto !important; /* 含义：允许表格跨页 */
  page-break-inside: auto !important; /* 含义：允许表格跨页 */
  margin: 8pt 0 12pt; /* 含义：PDF中表格上下间距 */
}} /* 结束 .table-wrap--wide */
.table-wrap--wide table {{ /* 含义：PDF宽表布局；设置：取消屏幕端最小宽度，固定到页面宽度 */
  width: 100% !important; /* 含义：宽表占满PDF可用宽度 */
  min-width: 0 !important; /* 含义：覆盖屏幕端宽表最小宽度 */
  max-width: 100% !important; /* 含义：限制在PDF页面内 */
  table-layout: fixed !important; /* 含义：稳定列宽，避免长文本撑开整表 */
  font-size: 7.3pt; /* 含义：PDF宽表基础字号 */
  line-height: 1.24; /* 含义：PDF宽表基础行高 */
}} /* 结束 .table-wrap--wide table */
.table-wrap--layout-wide-risk-assessment-table table {{ /* 含义：六列风险评估表；设置：保留比超宽明细表稍高可读性 */
  font-size: 7.8pt; /* 含义：风险评估表PDF字号 */
}} /* 结束 .table-wrap--layout-wide-risk-assessment-table table */
.table-wrap--wide caption {{ /* 含义：PDF宽表标题；设置：紧凑但清晰 */
  font-size: 8.2pt; /* 含义：表格标题字号 */
  line-height: 1.2; /* 含义：表格标题行高 */
  margin-bottom: 4pt; /* 含义：标题与表格间距 */
}} /* 结束 .table-wrap--wide caption */
.table-wrap--wide thead {{ /* 含义：PDF宽表表头；设置：跨页重复表头 */
  display: table-header-group; /* 含义：打印分页时重复表头 */
}} /* 结束 .table-wrap--wide thead */
.table-wrap--wide tr {{ /* 含义：PDF宽表行；设置：尽量避免单行被硬拆 */
  break-inside: avoid; /* 含义：避免行内分页 */
  page-break-inside: avoid; /* 含义：避免行内分页 */
}} /* 结束 .table-wrap--wide tr */
.table-wrap--wide th,
.table-wrap--wide td {{ /* 含义：PDF宽表单元格；设置：长文本更紧凑、更稳定地换行 */
  padding: 3.5pt 4pt !important; /* 含义：PDF宽表内边距 */
  vertical-align: top; /* 含义：长文本顶部对齐 */
  overflow-wrap: anywhere; /* 含义：允许长代号或长英文串在任意点换行 */
  word-break: normal; /* 含义：避免中文和英文被逐字硬拆 */
  hyphens: auto; /* 含义：英文长词自动连字符换行 */
}} /* 结束 .table-wrap--wide td */
.table-wrap--wide th {{ /* 含义：PDF宽表表头；设置：减少表头高度 */
  font-size: 7.2pt; /* 含义：表头字号 */
  line-height: 1.18; /* 含义：表头行高 */
}} /* 结束 .table-wrap--wide th */
.table-wrap--wide td p,
.table-wrap--wide th p {{ /* 含义：PDF宽表单元格段落；设置：移除全局段落空隙 */
  margin: 0; /* 含义：消除单元格内多余上下空白 */
  line-height: 1.24; /* 含义：单元格内部行高 */
}} /* 结束 .table-wrap--wide td p */
.table-wrap--wide td p + p,
.table-wrap--wide th p + p {{ /* 含义：PDF宽表连续段落；设置：轻量分隔 */
  margin-top: 2pt; /* 含义：连续段落间距 */
}} /* 结束 .table-wrap--wide td p + p */
/* 防止图片和图表溢出 */
img, canvas, svg {{ /* 含义：媒体元素尺寸限制；设置：在本块内调整相关属性 */
  max-width: 100% !important; /* 含义：最大宽度；设置：按需调整数值/颜色/变量 */
  height: auto !important; /* 含义：高度设置；设置：按需调整数值/颜色/变量 */
...
"""
    ...
    def render_from_markdown(
        ...
        """Render markdown to HTML with real heading/table parsing."""
        ...
        escaped_title = self._escape_html(title or "Report")
        return f"""<!DOCTYPE html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8" />
  ...
  <style>
    body {{ font-family: Georgia, "Times New Roman", serif; margin: 40px auto; max-width: 960px; color: #1f2937; line-height: 1.7; }}
    ...
    pre {{ background: #111827; color: #f9fafb; padding: 16px; overflow-x: auto; border-radius: 8px; }}
    blockquote {{ border-left: 4px solid #9ca3af; margin: 20px 0; padding: 4px 16px; color: #4b5563; }}
  </style>
</head>
<body>
{rendered_html}
...
function exportPdf() {
  ...
  try {
    if (window.pdfFontData) {
      ...
      pdf.addFont('SourceHanSerifSC-Medium.ttf', 'SourceHanSerif', 'bold');
...
def _render_chart(block: Any, chart_index: int = 0) -> str:
  ...
  if not isinstance(content, dict):
    return (
      f'<figure class="report-chart-figure chart-error" id="chart-{chart_index}-error">'
      f'<figcaption class="chart-title">chart-error</figcaption>'
      f'<div class="chart-error">Invalid chart payload</div></figure>'
    )

  chart_type = str(content.get("type") or "bar")
  title = str(content.get("title") or "Chart")
  caption = str(content.get("caption") or "")
  labels = content.get("labels") if isinstance(content.get("labels"), list) else []
  payload = {
    "config": {
      "type": chart_type,
      "data": {
  ...
  figure_id = f"chart-{chart_index}-figure"
  canvas_id = f"chart-{chart_index}-canvas"
  return (
    f'<figure class="report-chart-figure" id="{figure_id}">'
    f'<figcaption class="chart-title">{title}</figcaption>'
    ...
    f'window.__cassandraCharts.push({json.dumps(payload, ensure_ascii=False)});</script>'
    f'</figure>'
  )


def _render_wordcloud(block: Any, wc_index: int = 0) -> str:
  content = getattr(block, "content", None)
  metadata = getattr(block, "metadata", {}) if hasattr(block, "metadata") else {}
  if isinstance(block, dict):
    content = block.get("content")
    metadata = block.get("metadata", {}) or {}

  words = content if isinstance(content, list) else []
  figure_id = f"wordcloud-{wc_index}-figure"
  canvas_id = f"wordcloud-{wc_index}-canvas"
  title = metadata.get("title") or "Wordcloud"
  return (
    f'<figure class="report-wordcloud-figure" id="{figure_id}">'
    f'<figcaption class="wordcloud-title">{title}</figcaption>'
    f'<canvas id="{canvas_id}"></canvas>'
    f'<script>window.__cassandraWordclouds = window.__cassandraWordclouds || [];' 
    f'window.__cassandraWordclouds.push({json.dumps({"words": words}, ensure_ascii=False)});</script>'
    f'</figure>'
  )


__all__ = ["HTMLRenderer", "_render_chart", "_render_wordcloud"]
