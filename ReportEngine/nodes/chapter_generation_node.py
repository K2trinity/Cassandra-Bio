"""Compatibility ChapterGenerationNode for legacy ReportEngine path.

The modern implementation lives under src.engines.report_engine.nodes.chapter_generation_node,
while legacy tests and callers still rely on a sanitizer helper and constructor
signature from the old module.
"""
...
from src.engines.report_engine.ir import ENGINE_AGENT_TITLES, IRValidator
...
class ChapterGenerationNode:
    ...
    def _sanitize_table_block(self, block: Dict[str, Any]) -> None:
        ...
        for row in rows:
            ...
            # Scalar row payloads are normalized into one-cell rows.
